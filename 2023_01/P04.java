...
public class P04 {
    ...
    public static void main(String[] args) {
        Integer[] tasks = new Integer[] { 2, 2, 3, 3, 2, 4, 4, 4, 4, 4 };
        System.out.println(minimumRounds(tasks));
    }

    private static int minimumRounds(Integer[] tasks) {
        int minRounds = 0;

        if (tasks.length == 1)
            return minRounds - 1;

        // counting frequencies of each difixulty of task
        HashMap<Object, Integer> freq = new HashMap<>();

        // adding frequency count to map
        for (Integer subTasks : tasks) {
            freq.put(subTasks, freq.getOrDefault(subTasks, 0) + 1);
        }

        for (Integer coutnFreq : freq.values()) {
            if (coutnFreq == 1)
                return minRounds = -1;
            minRounds += (coutnFreq + 2) / 3;
        }
